...
export default class PixiResourcesLoader {
   ...
   */
  static getPIXITexture(project: gdProject, resourceName: string, isNew = false) {
  ...
  */
  static getPixiTextureRelativeToFile(project: gdProject, resourceName: string, relativeToFile: string){
    if (!project.getResourcesManager().hasResource(resourceName)) {
      const resourceRelativePath = path.join(path.dirname(relativeToFile), resourceName);
      ...
      console.log("ADDED RESOURCE:", resourceName, project.getResourcesManager().getAllResourceNames())
